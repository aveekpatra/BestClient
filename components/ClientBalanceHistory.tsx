...
export function ClientBalanceHistory({ clientId }: ClientBalanceHistoryProps) {
  ...
  const balanceTimeline = useQuery(api.balanceHistory.getClientBalanceTimeline, {
    clientId,
          ...
          {balanceTimeline.timeline.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                ...
                <div className={`text-right ${getBalanceColor(balanceTimeline.currentBalance)}`}>
                  <p className="text-lg font-bold">
                    {formatCurrency(Math.abs(balanceTimeline.currentBalance))}
                  </p>
                  <p className="text-xs">
                    {balanceTimeline.currentBalance > 0 ? 'Client Owes' : 
                     balanceTimeline.currentBalance < 0 ? 'You Owe' : 'Balanced'}
                  </p>
                </div>
              </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <div className={`font-medium ${getBalanceColor(entry.runningBalance)}`}>
                            {formatCurrency(Math.abs(entry.runningBalance))}
                          </div>
                          {entry.balanceChange !== 0 && (
                            <div className={`text-xs ${entry.balanceChange > 0 ? 'text-green-600' : 'text-red-600'}`}>
                              {entry.balanceChange > 0 ? '+' : ''}{formatCurrency(Math.abs(entry.balanceChange))}
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      {entry.workDetails && (
                        <div className="mt-2 p-2 bg-gray-50 rounded text-xs">
                          <div className="flex items-center justify-between">
                            <span className="font-medium">{entry.workDetails.description}</span>
                            <Badge variant="outline">
                              {getWorkTypeLabel(entry.workDetails.workType)}
                            </Badge>
                          </div>
                          <div className="flex items-center justify-between mt-1 text-gray-500">
                            <span>Total: {formatCurrency(entry.workDetails.totalPrice)}</span>
                            <span>Paid: {formatCurrency(entry.workDetails.paidAmount)}</span>
                            <Badge className={getPaymentStatusInfo(entry.workDetails.paymentStatus).colorClass}>
                              {getPaymentStatusInfo(entry.workDetails.paymentStatus).label}
                            </Badge>
                          </div>
                        </div>
                      )}
              ...
              {balanceTimeline.timeline.length > 10 && (
                <div className="text-center">
                  ...
                  >
                    {showFullHistory ? 'Show Less' : `Show All ${balanceTimeline.totalEntries} Entries`}
                  </Button>
                </div>
              )}
            </div>
                  ...
                  <TableRow key={entry._id}>
                    ...
                    <TableCell>
                      <span className={getBalanceColor(entry.previousBalance)}>
                        {formatCurrency(Math.abs(entry.previousBalance))}
                      </span>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center space-x-1">
                        ...
                        <span className={entry.balanceChange > 0 ? 'text-green-600' : entry.balanceChange < 0 ? 'text-red-600' : 'text-gray-600'}>
                    ...
                    <TableCell>
                      <span className={getBalanceColor(entry.newBalance)}>
                        {formatCurrency(Math.abs(entry.newBalance))}
                      </span>
                    </TableCell>
                  </TableRow>
                ))}
