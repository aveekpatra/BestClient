...
export class PDFReceiptGenerator {
  ...
  private addBusinessHeader(businessInfo: ReceiptData['businessInfo']) {
    ...
    this.doc.text(businessInfo.name, this.pageWidth / 2, this.currentY, { align: 'center' });
    this.currentY += 10;

    // Business details
    this.doc.setFontSize(10);
    this.doc.setFont('helvetica', 'normal');
    this.doc.text(businessInfo.address, this.pageWidth / 2, this.currentY, { align: 'center' });
    this.currentY += 6;
    ...
    if (businessInfo.gst) {
      this.doc.text(`GST No: ${businessInfo.gst}`, this.pageWidth / 2, this.currentY, { align: 'center' });
      this.currentY += 6;
    }
  ...
  private addReceiptHeader(receiptNumber: string, date: string) {
    ...
    this.doc.setFont('helvetica', 'bold');
    this.doc.text('RECEIPT', this.pageWidth / 2, this.currentY, { align: 'center' });
    this.currentY += 15;

    this.doc.setFontSize(10);
    this.doc.setFont('helvetica', 'normal');
    this.doc.text(`Receipt No: ${receiptNumber}`, this.margin, this.currentY);
    this.doc.text(`Date: ${date}`, this.pageWidth - this.margin, this.currentY, { align: 'right' });
    this.currentY += 15;
  }
  ...
  private addWorkTable(works: Work[]) {
    ...
    works.forEach((work, index) => {
      ...
      if (index % 2 === 1) {
        ...
        this.doc.rect(this.margin, rowY, this.pageWidth - 2 * this.margin, 8, 'F');
      }

      this.doc.text(work.transactionDate, colPositions[0] + 2, rowY + 6);
      
      // Truncate description if too long
      const description = work.description.length > 25 ? 
        work.description.substring(0, 22) + '...' : work.description;
      this.doc.text(description, colPositions[1] + 2, rowY + 6);
      
      this.doc.text(getWorkTypeLabel(work.workType), colPositions[2] + 2, rowY + 6);
      this.doc.text(formatCurrency(work.totalPrice), colPositions[3] + 2, rowY + 6);
      this.doc.text(formatCurrency(work.paidAmount), colPositions[4] + 2, rowY + 6);
  ...
  private addTotals(totalAmount: number, totalPaid: number) {
    ...
    this.doc.text(formatCurrency(totalAmount), rightAlign, this.currentY, { align: 'right' });
    this.currentY += 6;
    
    // Total Paid
    this.doc.text('Total Paid:', rightAlign - 60, this.currentY);
    this.doc.text(formatCurrency(totalPaid), rightAlign, this.currentY, { align: 'right' });
    this.currentY += 6;
    
    // Balance
    this.doc.setFont('helvetica', 'bold');
    const balanceLabel = balance > 0 ? 'Balance Due:' : balance < 0 ? 'Overpaid:' : 'Balance:';
    this.doc.text(balanceLabel, rightAlign - 60, this.currentY);
    this.doc.text(formatCurrency(Math.abs(balance)), rightAlign, this.currentY, { align: 'right' });
    
    // Horizontal line above totals
    this.doc.setLineWidth(0.5);
    this.doc.line(rightAlign - 80, this.currentY - 20, rightAlign, this.currentY - 20);
  ...
  private addFooter() {
    ...
    this.doc.text('This is a computer generated receipt.', this.pageWidth / 2, footerY + 6, { align: 'center' });
  }
  ...
  generateMultiClientReceipt(data: ReceiptData): string {
    ...
    works.forEach(work => {
      ...
      groupedWorks[work.clientId].totalBalance += (work.totalPrice - work.paidAmount);
    });
    ...
    this.doc.setFontSize(12);
    this.doc.setFont('helvetica', 'bold');
    this.doc.text('GRAND TOTAL', this.pageWidth / 2, this.currentY, { align: 'center' });
    this.currentY += 10;
...
export function generateReceiptPDF(
  ...
  const clientName = isMultiClient 
    ? 'Multi-Client' 
    : clients[works[0]?.clientId]?.name?.replace(/[^a-zA-Z0-9]/g, '-') || 'Unknown';
